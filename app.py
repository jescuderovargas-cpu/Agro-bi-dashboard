...
# --- ESTILO LIMPIO: CONTRASTE ALTO Y SIN SOMBRAS EN MÓVIL ---
st.markdown("""
    <style>
...
if df_raw is not None:
    ...
    # DataFrame Operativo (Excluyendo mermas y reclamaciones)
    ...
    kpi2.metric("Precio Venta Medio", f"{form(pm_vta, 3)} €/kg")
    kpi3.metric("Margen Neto Final", f"{form(margen_kg_global, 4)} €/kg")
    st.markdown("---")

    tabs = st.tabs(["RESUMEN DE NEGOCIO", "SEGUNDAS Y TIRADO", "RECLAMACIONES"])

    with tabs[0]:
        st.markdown("### Resumen de Precios y Volumen")
        f1, f2, f3 = st.columns(3)
        f1.metric("Facturación Neta", f"{form(t_vta)} €")
        f2.metric("P. Medio Compra", f"{form(pm_com, 3)} €/kg")
        ...
        st.markdown("### Gastos Operativos (€/kg)")
        ...
        dg4.metric("Transporte", f"{form((df_op['porte_orig'].sum() + df_op['porte_dest'].sum())/t_kg_e if t_kg_e>0 else 0, 3)} €/kg")
        dg5.metric("Comisión", f"{form(df_op['comision'].sum()/t_kg_e if t_kg_e>0 else 0, 3)} €/kg")

        st.markdown("---")
        # ... (Resto de tus gráficos de Plotly se mantienen igual)
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.plotly_chart(px.bar(df_op.groupby('familia')['venta_neta'].sum().reset_index(), x='familia', y='venta_neta', color='familia'), use_container_width=True)
        with c2:
            st.plotly_chart(px.pie(df_op, values='pesonetovendido', names='familia', hole=0.4), use_container_width=True)

    with tabs[1]:
        # Sección de Segundas y Tirado (como la tenías)
    ...
    with tabs[2]:
        st.markdown("### Impacto de Reclamaciones")
        st.metric("Total Abonado", f"{form(df_reclamaciones['venta_neta'].sum())} €")
